...
function deriveStatus(deliveryDate: string, markedComplete: boolean): 'completed' | 'scheduled' {
  if (markedComplete || (deliveryDate && deliveryDate < TODAY)) return 'completed'
  return 'scheduled'
}
            ...
            <div className="flex flex-col justify-end gap-2 pb-1">
              <span className={cn(
                ...
                derivedStatus === 'completed'
                  ? 'bg-fleet-active-bg text-fleet-active'
                  : 'bg-fleet-maintenance-bg text-fleet-maintenance'
              )}>
                {derivedStatus === 'completed' ? 'Reparado' : 'En Reparacion'}
              </span>
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                />
                <span className="text-[12px] text-text-secondary">Marcar como reparado</span>
              </label>
            </div>
          </div>
